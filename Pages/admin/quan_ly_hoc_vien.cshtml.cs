...
{
    public class quan_ly_hoc_vienModel : AdminBasePageModel
    {
        private readonly du_lieu _db;

        public quan_ly_hoc_vienModel(du_lieu db)
        {
            _db = db;
        }
        ...
        {
            var hocVienIds = _db.nguoi_dung_vai_tro
                .Where(v => v.ma_vai_tro == "user")
                .Select(v => v.ma_nguoi_dung)
                .ToList();

            DanhSach = _db.nguoi_dung
                .Where(u => hocVienIds.Contains(u.ma_nguoi_dung))
